    ...
    <div className="px-5 py-4 border-b border-white/6">
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <span className="text-xs font-medium text-white/50">Slides this week</span>
          <span className="text-xs font-semibold text-white/80">{totalGenerated}</span>
        </div>
        ...
        {data.map((d, i) => {
          ...
          return (
            <div key={d.day} className="flex-1 flex flex-col items-center gap-1 group/bar">
              ...
              <span className={`text-[9px] font-medium ${isToday ? "text-indigo-400" : "text-white/25"}`}>
                {d.day}
              </span>
            </div>
          );
  ...
  return <>
    <div className="p-4 sm:p-6 lg:p-8 w-full max-w-7xl mx-auto">
            ...
            <div className="relative shrink-0">
              <div className="w-9 h-9 sm:w-11 sm:h-11 rounded-lg sm:rounded-lg flex items-center justify-center text-white font-bold text-sm sm:text-base">
                <Avatar className="h-8 w-8 rounded-lg">
                  <AvatarImage src="https://github.com/maxleiter.png" alt="logo" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-5">
        ...
        <div className="lg:col-span-2 bg-[#002137] border border-white/8 rounded-2xl overflow-hidden">
          ...
          <div className="flex items-center justify-between px-5 py-4 border-b border-white/6">
            <div className="flex items-center gap-2.5">
              ...
              <h2 className="text-sm font-semibold text-white tracking-tight">Recent Presentations</h2>
          ...
          {recentPresentations.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 px-6 text-center">
              ...
              <p className="text-sm font-medium text-white/40">No presentations yet</p>
              <p className="text-xs text-white/20 mt-1 mb-4">Create your first to get started</p>
              <button
                onClick={() => navigate("/dashboard/slide")}
            ...
            <div className="divide-y divide-white/4">
              {/* Column headers (sm+) */}
              <div className="hidden sm:grid grid-cols-[1fr_64px_72px_96px] text-[10px] font-semibold text-white/25 uppercase tracking-widest px-5 py-2.5">
                <span>Name</span>
                <span>Slides</span>
                <span>Date</span>
                <span></span>
              ...
              {recentPresentations.map((p) => (
                <div key={p.id} className="group relative hover:bg-white/3 transition-colors duration-150">
                  ...
                  <div className="hidden sm:grid grid-cols-[1fr_64px_72px_96px] items-center px-5 py-3.5">
                    ...
                    <div className="flex items-center gap-1">
                      <span className="text-sm text-white/40">{p.slides}</span>
                    </div>
                    <span className="text-xs text-white/35 font-medium">{p.date}</span>
                    <div className="flex gap-1.5 opacity-0 group-hover:opacity-100 translate-x-1 group-hover:translate-x-0 transition-all duration-150">
                      <button className="px-2.5 py-1 text-[10px] font-semibold border border-white/10 rounded-lg text-white/40 hover:text-white/70 hover:border-white/20 hover:bg-white/5 transition-all">
                        .pptx
                      </button>
                      <button className="px-2.5 py-1 text-[10px] font-semibold border border-white/10 rounded-lg text-white/40 hover:text-white/70 hover:border-white/20 hover:bg-white/5 transition-all">
                        .pdf
                  ...
                  <div className="flex sm:hidden items-center gap-3 px-4 py-3.5">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        <span className="text-[11px] text-white/30">{p.slides} slides</span>
        ...
        <div className="bg-[#002137] border border-white/8 rounded-2xl overflow-hidden flex flex-col">
          ...
          <div className="flex items-center justify-between px-5 py-4 border-b border-white/6">
            ...
            <button className="group flex items-center gap-1 text-[11px] font-medium text-blue-400/70 hover:text-blue-400 transition-colors">
              View all
              <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="group-hover:translate-x-0.5 transition-transform duration-150">
          ...
          {/* ── Activity List ── */}
          <div className="flex flex-col divide-y divide-white/4 flex-1">
                ...
                <div className="flex-1 min-w-0 pb-0.5">
                  <p className="text-xs sm:text-sm text-white/70 leading-snug">
                    <span
                      ...
                      style={{ color: a.accent, background: a.accentBg }}
                    >
                      {a.label}
                    </span>
                    <span className="font-medium text-white/90">{a.subject}</span>
                    {a.context && (
                      <span className="text-white/40"> · {a.context}</span>
                    )}
                  </p>
                  <p className="text-[10px] text-white/25 mt-1 font-medium">{a.time}</p>
                </div>
              </div>
            ))}
          </div>
            ...
            <div className="flex items-center gap-1">
              {activities.map((a) => (
                <div
                  ...
                  style={{ background: a.accentBg, border: `1px solid ${a.accent}40` }}
                />
              ))}
            </div>
          </div>
